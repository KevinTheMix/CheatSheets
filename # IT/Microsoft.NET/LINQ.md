...
* `Enumerable.GroupBy()` (deferred) = groups the elements of a sequence
* `Enumerable.ToDictionary()` = maps one key -> one value (1:1), [mutable](https://stackoverflow.com/a/5659080/3559724)
* `Enumerable.ToLookup()` (immediate) = maps one key -> n values (1:n), unmutable, querying a key that does not exist returns an empty sequence (w/o error)
