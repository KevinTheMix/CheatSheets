...
* **Mono** = FOSS implementation of .NET (sponsored by _Microsoft_, now superseded by .NET itself)
...
* **MediatR** = Mediator Pattern implementation in .NET (ie requests are classes handled by ad hoc genericly-typed handlers with DI discovery)
...
* [JSONSharp](https://jsontocsharpconverter.web.app) = convert JSON to C# Object Initializer Syntax
...
* **MSMQ** (Microsoft Message Queuing) = Windows-only native message queueing (between applications/machines) system (uses disk-backed queues for durability, mostly considered legacy)
