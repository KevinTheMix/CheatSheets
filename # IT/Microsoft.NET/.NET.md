...
* **Office Interop** = COM-based APIs (actually ABIs, with IPC) Microsoft exposes for Office apps, controls Excel's object model (workbooks, sheets, ranges, UI)
...
* **WinRT** (Windows Runtime) = platform-agnostic application platform/API layer (that can be consumed from C++/C€/etc)
* **WinUI** = UI framework built on top of Windows technologies with XAML, controls, styling, Fluent Design system for building Windows UI
...
* **Excel-DNA** = independent open source library/NuGet/toolkit for building Microsoft Excel add-ins (eg functions, commands, ribbons) in C#/VB.NET
  * User-Defined Function (UDF) = custom .NET function callable from Excel
