...
* **Aspire** = development-time healthcheck/observable/orchestration/telemetry framework for cloud-native distributed microservice applications (DI-powered string-based name resolution discoverability at startup, a dashboard)
  * Consists of a special _AppHost_ console project that replaces Docker compose file for local development, spins up any dependencies (declaratively described purely via C# code) as containers when launched
  * Comes along with a built-in developer dashboard that shows logs/traces/metrics across all services in one place (arguably one of its most useful features)
