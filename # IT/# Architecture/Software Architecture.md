...
* **Conway's Law** = an application will reflect social boundaries of organization that produced it
...
* [Behavioral patterns](https://en.wikipedia.org/wiki/Behavioral_pattern) = design patterns that identify common comunication patterns among objects (eg Memento, Observer)
  * **Command** = uses an object to encapsulate all information needed to perform an action/trigger an event at a later time (ie method name, method owner object, method parameter values)
