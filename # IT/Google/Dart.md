...
* **Dart Runtime** = optimized environment in which Dart code executes (whether AOT or JIT), manages memory (GC) & concurrency (isolates), enforces (dynamic-time) typing
  * Part of Dart VM (JIT), it is automatically included in AOT-compiled self-contained executables (eg _.exe_ files) on native platforms
...
* **Dart VM** = platform enabling JIT incremental recompilation, provided by `dart run` CLI command, which launches a dedicated process (eg _dart.exe_ on Windows)
* **Isolate** = independant unit of execution that does not share memory & communicate via messages (inspired by Erlang), making it safer for concurrency compared to thread-based models
...
* `_` (entire identifier) = a private constructor (eg for singleton pattern)
...
* `extension`= extension method (eg `extension KokoExtension on TargetType { return_type koko() { … this … } }`)
* `external` = [separates function declaration and body](https://stackoverflow.com/a/24929907/3559724), also a way to include abstract functions to non-abstract classes
...
* `static` = can be applied to class methods, and be called via `Class.method()`
...
* `print()` = built-in function to output text to console (for debugging/logging)
...
* Types = can be used as variables (via **Type** class eg `Type kokoType = int`, à la reflection)
  * `variable.runtimeType` = à la **runtime** `typeof`

### [Operators](https://dart.dev/language/operators)

* `a!` = casts nullable _a_ to its underlying non-nullable type
...
* `??=` = assign value if receiver was null
...
* Note: adding/removing will not work if list is constant (eg `const list = []`), or its rvalue is a constant literal (eg `var list = const []`, in that case however `list` can be reassigned since it's not `const` - unless it is `final` too)
...
* [Constructors](https://www.freecodecamp.org/news/constructors-in-dart)
  ...
  * `Koko._(…);` or `Koko._(…) {…}` = makes unnamed constructor private => can only instantiate via other **named** constructors
  * `Koko(…): this.other(a, b)` = calling a named constructor
  * `Koko.other(…): this(a, b)` = calling unnamed constructor
