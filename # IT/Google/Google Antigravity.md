...
* **Rules** = always-on manually defined constraints (not capabilities) for agent to follow (as a Markdown file)
  ...
  * _./agent/rules_ = workspace rules
* **Task List** = artifact used by agent to approach complex tasks, keep track of user's end-goal & monitor progress on various action items (ie a markdown list snapshot of what an agent is doing)
* **Workflow** = user-initiated (via `/workflow-name`) triggerable prompt (series of steps, saved as a Markdown file) to guide an agent through a repetitive set of tasks (eg deploying a service, responding to PR comments)

## CLI

## Shortcuts

* `Ctrl + E` = Agent Manager
* `Ctrl + L` = toggle Agent View
